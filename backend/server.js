...
// Get Leaderboard (Top scores for everyone)
...
// [NEW] Get User Specific Results (For User Dashboard)
app.get('/api/results/:username', async (req, res) => {
  try {
    const results = await Result.find({ username: req.params.username })
      .populate('quizId', 'title subject') // This gets the Quiz Title automatically
      .sort({ date: -1 }); // Show newest attempts first
    res.json(results);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
